...
<div class="card">
    <h5 class="card-header">Brendni tahrirlash</h5>
    <div class="card-body">
      <form method="post" action="{{route('brand.update',$brand->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($brand->status=='active') ? 'selected' : '')}}>Aktiv</option>
            <option value="inactive" {{(($brand->status=='inactive') ? 'selected' : '')}}>Aktiv emas</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
